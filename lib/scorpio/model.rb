...
module Scorpio
  ...
  class Model
    ...
    # the class on which the swagger document is defined. subclasses use the swagger document set on this class
    ...
    define_inheritable_accessor(:definition_keys, default_value: [], update_methods: true, on_set: proc do
      definition_keys.each do |key|
        ...
        schema_as_key = schema_as_key.object if schema_as_key.is_a?(Scorpio::Swagger::Schema)
        schema_as_key = schema_as_key.content if schema_as_key.is_a?(Scorpio::JSON::Node)

        swagger_document_class.models_by_schema = swagger_document_class.models_by_schema.merge(schema_as_key => self)
      end
    end)
    ...
    class << self
      def set_swagger_document(swagger_document)
        if swagger_document.is_a?(Hash)
          swagger_document = Swagger::Document.new(swagger_document)
        end
        swagger_document.paths.each do |path, path_item|
          path_item.each do |http_method, operation|
            ...
            unless operation.is_a?(Scorpio::Swagger::Operation)
      ...
      def method_names_by_operation
        @method_names_by_operation ||= Hash.new do |h, operation|
          h[operation] = begin
            raise(ArgumentError, operation.pretty_inspect) unless operation.is_a?(Scorpio::Swagger::Operation)
